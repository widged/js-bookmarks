...
var BookmarkExplorerPrivate = (function () {
  function BookmarkExplorerPrivate(_ref) {
    ...
    importer.load('etc/data/js-assets.tsv', function (_ref2) {
      var db = _ref2.db;
      ...
      xhr(file, function (tsv) {
        ...
        var db = tsv.split("\n").reduce(function (acc, d, i) {
          if (d.length) {
            ...
            console.log("TRM", terms);
            console.log("OTR", others);
            tags = (tags || "").split(";");
            ...
            others = (others || "").split(";");
            var fmt = "";
            var doc = { src: src, name: name, fmt: fmt, tags: tags, terms: terms, others: others };
            acc.push(doc);
          }
          return acc;
        }, []);
